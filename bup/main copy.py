...
# エイリアスを使用して、グローバルスコープでの直接呼び出しを避ける
...
# --- Admin SDK と Firestore Client の遅延初期化のための準備 ---
_admin_app_initialized = False
db = None  # Firestoreクライアントのグローバルインスタンス

def initialize_services_if_needed():
    """
    Firebase Admin SDK と Firestore クライアントを必要に応じて初期化する。
    Admin SDKの初期化は一度だけ行われる。
    Firestoreクライアントも、Admin SDK初期化後に一度だけ取得される。
    """
    global _admin_app_initialized, db

    if not _admin_app_initialized:
        try:
            _initialize_app()
            print("Admin SDK initialized on demand.")
            _admin_app_initialized = True
        except Exception as e:
            print(f"Failed to initialize Admin SDK on demand: {e}")
            # 初期化失敗時は以降の処理が困難なため、エラーを再送出することも検討
            # この例では、関数の呼び出し元で db is None のチェックを期待する
            return  # db の初期化もスキップ

    # Admin SDKが正常に初期化された場合のみ、Firestoreクライアントを取得試行
    if _admin_app_initialized and db is None:
        try:
            db = _firestore.client()
            print("Firestore client obtained on demand.")
        except Exception as e:
            print(f"Failed to get Firestore client on demand: {e}")
            # db が None のままになる。呼び出し元でのチェックが必要。
            pass
...
def hello_world(req: https_fn.Request) -> https_fn.Response:
    ...
    if db is None:
        print("Error in hello_world: Firestore client not initialized or unavailable.")
    ...
    try:
        ...
        test_data = {
            ...
            "timestamp": _firestore.SERVER_TIMESTAMP  # エイリアス経由で使用
        }
...
def generate_or_fetch_api_key(req: https_fn.Request) -> https_fn.Response:
    ...
    if db is None:
        print("Error in generate_or_fetch_api_key: Firestore client not initialized or unavailable.")
        return https_fn.Response("Server configuration error: Firestore unavailable.", status=500)
    ...
    try:
        # 2. IDトークンを検証
        try:
            decoded_token = _auth.verify_id_token(id_token)  # エイリアス経由で使用
        except (_auth.InvalidIdTokenError, _auth.ExpiredIdTokenError, ValueError) as auth_error:
            # トークン検証失敗
        ...
        keys_ref = db.collection("apiKeys")
        # ユーザーUIDと有効フラグで検索
        query = keys_ref.where(
            field_path="user_uid", op_string="==", value=uid
        ).where(
            field_path="isEnabled", op_string="==", value=True
        ).limit(1)
        docs = list(query.stream())
        ...
        if docs:
            ...
            # 新しいAPIキー文字列を生成
            new_api_key_string = generate_api_key_string_internal()
            if len(new_api_key_string) > 5:
                api_key_short = new_api_key_string[:5] + "..."
            else:
                api_key_short = new_api_key_string
            ...
            new_key_data = {
                ...
                # "expires_at": _firestore.SERVER_TIMESTAMP + timedelta(days=7) # 例:有効期限
            }
            ...
            try:
                ...
                return https_fn.Response(new_api_key_string, status=201, content_type="text/plain")
