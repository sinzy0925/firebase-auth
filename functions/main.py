...
logging.basicConfig(level=log_level, format='%(levelname)s: %(asctime)s: %(message)s') # 時刻も追加
...
def verify_api_key(req: https_fn.Request) -> https_fn.Response:
    ...
    注意: この関数は冪等性を持ちません。複数回呼び出されると都度カウントアップされる可能性があります。
    ...
    try:
        ...
        key_data: dict = key_doc_snapshot.to_dict() # to_dict() can return None if doc exists but no data
        if key_data is None: # Should not happen with Firestore but good to check
        ...
        def check_and_update_usage_in_transaction(
            ...
            if current_data is None: # Should not happen with Firestore but good to check
...
def record_api_usage(req: https_fn.Request) -> https_fn.Response:
    ...
    except Exception as body_parse_error: # Should not happen with silent=True
        return create_error_response(
    ...
    try:
        ...
        if processed_txn_doc.exists:
            ...
            if processed_data is None: # Should not happen
                 processed_data = {}
...
def generate_or_fetch_api_key(req: https_fn.Request) -> https_fn.Response:
    ...
    try:
        ...
        if active_key_docs:
            ...
            if existing_key_data is None: # Should not happen
